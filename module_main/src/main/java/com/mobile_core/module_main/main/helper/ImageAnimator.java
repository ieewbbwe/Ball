package com.mobile_core.module_main.main.helper;
...
import com.mobile_core.base.utils.BindingUtils;
...
public class ImageAnimator {
    ...
    ImageAnimator(CollapsingToolbarLayout collapsingToolbar, ImageView targetImage, ImageView outgoingImage) {
        ...
        Resources mResources = collapsingToolbar.getResources();
        int[] mColorDay = mResources.getIntArray(R.array.day_color_tab);
        int[] mColorNight = mResources.getIntArray(R.array.night_color_tab);
        mColors = SharedPrefManager.getBoolean(Constants.SP_KEY_IS_NIGHT,false) ? mColorNight : mColorDay;
    ...
    void forward(int position, float positionOffset) {
        ...
        int color = Utiles.evaluate(positionOffset, mColors[position], mColors[position + 1]);
    ...
    void backwards(int position, float positionOffset) {
        ...
        int color = Utiles.evaluate(1 - positionOffset, mColors[position + 1], mColors[position]);
        collapsingToolbar.setContentScrimColor(color);
        collapsingToolbar.setStatusBarScrimColor(color);
