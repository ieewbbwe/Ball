...
public class LoginActivity extends MActivity<LoginViewModel, ActivityLoginBinding>
    ...
    @Override
    public void countDownCallBack(boolean enable, int countTime) {
        binding.reqCode.setEnabled(enable);
        if (enable) {
            binding.reqCode.setText(R.string.request_validate_code);
        } else {
            binding.reqCode.setText(String.valueOf(countTime));
        }
    }
}
